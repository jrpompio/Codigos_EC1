...
jal EXIT         # Ejecutando la salida del programa
...
# $v1: registro donde retorna el valor máximo
	...
	LOOP0:           	 # Loop para control del puntero
		...
		lw $t0, 0($t7)   # se carga elemento a contar
		addi $t8, $t8, 4 # este elemento ocupa espacio de 1 word
		addi $t7, $t7, 4 # se analizará el siguiente elemento
	bne $t0, $0, LOOP0   # si el elemento presente no es cero se repite
...
# reciclando el codigo de busqueda anterior
#--------------------------------------------------------------------------------
	...
	sub $t1, $t7, $sp 		  # desfase con el puntero 
	addi $t7, $t7, 4          # Aumenta en 4 para encontrar el indice siguiente
	bne $t0, $0, LOOP4        # como el buscador termine al encontrar el primer cero
							  # el desfase almacenado en $t1
							  # se le suma a la dirección del primer elemento del Array
