.data 
    cadena: .asciiz "aba"    # Definición de cadenas de texto
	msj_si: .asciiz "Es palindromo"
	msj_no: .asciiz "No es palindromo"
.text
main: # Codigo principal
 ...
 j end # Se salta hasta el final de la sección de funciones para no pasar por ellas
 ...
 upper: # Función para cambiar minusculas por mayusculas
 	...
 	sub $t7 , $t7, $t7 # Se inicializa $t7 en cero, es un contador
 ...
 palindromo: # Función que identifica si es un palindromó o no
	...
	j si # se no se encontró ninguna comparación que indique
		 # que no es un palindromo, se salta al valor que indica 
		 # que si lo es $s1 = 1
 ...
 end: # final después de la sección de funciones, que se usa para no pasar por ellas 
